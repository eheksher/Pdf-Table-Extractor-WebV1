// PDF Extractor - Complete JavaScript with Panel Controls and 5 File Limit
// גרסה מלאה ומתוקנת עם בקרת פאנלים והגבלה ל-5 קבצים
...
function addFloatingWindowStyles() {
  ...
  style.textContent = `
    ...
    /* סטיילים להתראות על הגבלת קבצים */
    .file-limit-warning {
      background: #fff3cd;
      color: #856404;
      padding: 10px;
...
// *** פונקציה מעודכנת עם הגבלת קבצים ***
function handleFiles(fileList, addToExisting = false) {
  ...
  // בדיקת הגבלת קבצים
  if (addToExisting) {
    // במצב הוספה - בדיקה שלא חורגים מ-5
    const existingCount = files.length;
    const availableSlots = MAX_FILES - existingCount;
    
    if (availableSlots <= 0) {
      showToast(`לא ניתן להוסיף קבצים נוספים. הגבלה: ${MAX_FILES} קבצים מקסימום`);
      return;
    }
    
    if (newFiles.length > availableSlots) {
      showToast(`ניתן להוסיף רק ${availableSlots} קבצים נוספים. נבחרו ${newFiles.length} קבצים`);
      return;
    }
  } else {
    // במצב החלפה - בדיקה שלא מעלים יותר מ-5
    if (newFiles.length > MAX_FILES) {
      showToast(`ניתן לטעון מקסימום ${MAX_FILES} קבצים בו-זמנית. נבחרו ${newFiles.length} קבצים`);
      return;
    }
  }
  ...
  if (addToExisting && files.length > 0) {
    ...
    // הוספת הקבצים החדשים לרשימה הקיימת
    files = files.concat(filesToAdd);
    
    // יצירת שורות נתונים עבור הקבצים החדשים
    ...
    // הוספת השורות החדשות לנתונים הקיימים
    data = data.concat(newRows);
    showToast(`נוספו ${filesToAdd.length} קבצים חדשים (סה״כ: ${files.length}/${MAX_FILES})`);
    ...
    selectedRows = [];
    showToast(`נטענו ${files.length}/${MAX_FILES} קבצים חדשים`);
  }
  
  // ===== ניקוי ועדכון (משותף לשני המצבים) =====
  ...
  updateSearchInfo();
  renderTable();
  updateFileCounter(); // עדכון מונה הקבצים
  
  // טעינת PDF ראשון להצגה
  if (files.length > 0) {
    if (addToExisting && typeof filesToAdd !== 'undefined' && filesToAdd.length > 0) {
...
// *** פונקציה חדשה לעדכון מונה הקבצים ***
function updateFileCounter() {
  // עדכון מונה הקבצים בממשק (אם קיים)
  const counter = safeGetElement('fileCounter');
  if (counter) {
    counter.textContent = `${files.length}/${MAX_FILES} קבצים`;
    
    // שינוי צבע אם מתקרבים להגבלה
    if (files.length >= MAX_FILES - 1) {
      counter.style.background = '#ffeb3b';
      counter.style.color = '#795548';
  ...
  // הצגת אזהרה אם מתקרבים להגבלה
  if (files.length >= MAX_FILES - 1 && files.length < MAX_FILES) {
    showFileWarning(`אזהרה: ניתן להוסיף עוד ${MAX_FILES - files.length} קובץ בלבד`);
  } else if (files.length >= MAX_FILES) {
    showFileWarning(`הגענו להגבלה המקסימלית של ${MAX_FILES} קבצים`);
  }
}

// *** פונקציה חדשה להצגת אזהרות קבצים ***
function showFileWarning(message) {
  // הסרת אזהרה קיימת
  const existingWarning = document.querySelector('.file-limit-warning');
  if (existingWarning) {
  ...
  // הוספת האזהרה לפני הטבלה
  const tableContainer = safeGetElement('tableContainer');
  if (tableContainer && tableContainer.parentNode) {
    tableContainer.parentNode.insertBefore(warning, tableContainer);
    
    // הסרת האזהרה אחרי 5 שניות
    setTimeout(function() {
...
function renderTable() {
  ...
  if (!dataToRender || dataToRender.length === 0) {
    ...
    if (searchTerm) {
      ...
      emptyHtml += '<div class="file-counter" id="fileCounter">0/' + MAX_FILES + ' קבצים</div>';
    }
    emptyHtml += '</div>';
  ...
  // הוספת מונה קבצים בתחתית הטבלה
  if (files.length > 0) {
    html += '<div style="text-align: center; margin-top: 10px;">';
    html += '<span class="file-counter" id="fileCounter">' + files.length + '/' + MAX_FILES + ' קבצים טעונים</span>';
    html += '</div>';
  }
  ...
  setTimeout(function() {
    applyColumnWidths();
    updateFileCounter(); // עדכון הסטיילים של המונה
  }, 10);
...
function deleteSelectedRows() {
  ...
  if (confirm('האם למחוק ' + selectedRows.length + ' שורות נבחרות?')) {
    ...
    updateSearchInfo();
    renderTable();
    updateFileCounter(); // עדכון מונה הקבצים אחרי מחיקה
  ...
  try {
    ...
    if (stored) {
      ...
      if (savedData.data && savedData.data.length > 0) {
        // הגבלת הנתונים שנטענים ל-5 קבצים מקסימום
        if (savedData.data.length > MAX_FILES) {
          savedData.data = savedData.data.slice(0, MAX_FILES);
          showToast(`נטענו רק ${MAX_FILES} קבצים ראשונים מהשמירה (הגבלה: ${MAX_FILES} מקסימום)`);
        }
...
function clearStoredData() {
  if (confirm('האם למחוק את כל הנתונים השמורים?')) {
    ...
    data = [];
    files = []; // איפוס גם את רשימת הקבצים
    selectedRows = [];
    filteredData = [];
    currentFileIndex = 0;
    updateSearchInfo();
...
function deleteFile(fileIndex) {
  ...
  if (confirm('האם למחוק את הקובץ "' + fileName + '"?')) {
    // מחיקת הקובץ והנתונים
    files.splice(fileIndex, 1);
    data.splice(fileIndex, 1);
    
    // עדכון אינדקס הקובץ הנוכחי
    if (currentFileIndex >= fileIndex && currentFileIndex > 0) {
      currentFileIndex--;
    }
    
    // אם מחקנו את הקובץ הנוכחי, טען אחר
    if (files.length > 0) {
      if (currentFileIndex >= files.length) {
        currentFileIndex = files.length - 1;
      }
      loadPdf(files[currentFileIndex]);
    } else {
      // אין קבצים - נקה את תצוגת ה-PDF
      const pdfContainer = safeGetElement('pdfContainer');
      if (pdfContainer) {
        pdfContainer.innerHTML = '<div class="loading">אין קובץ PDF טעון</div>';
      }
      currentPdf = null;
    }
    ...
    showToast('קובץ "' + fileName + '" נמחק בהצלחה');
  }
}

// *** פונקציה לקבלת מידע על הגבלת הקבצים ***
...
// *** פונקציה לבדיקה אם ניתן להוסיף קבצים ***
function canAddFiles(count = 1) {
  return files.length + count <= MAX_FILES;
}

// *** פונקציה להצגת סטטוס הקבצים ***
function showFileStatus() {
  const info = getFileLimit();
  let message = `קבצים טעונים: ${info.current}/${info.max}`;
  
  if (info.remaining > 0) {
    message += ` (ניתן להוסיף עוד ${info.remaining})`;
  } else {
    message += ' (הגעת להגבלה המקסימלית)';
  }
  
  showToast(message, 4000);
}

// *** עדכון הפונקציה לניקוי קבצים עם בדיקת הגבלה ***
function clearAllFiles() {
  if (files.length === 0) {
    showToast('אין קבצים למחיקה');
    return;
  }
  
  if (confirm(`האם למחוק את כל ${files.length} הקבצים?`)) {
    files = [];
    data = [];
    selectedRows = [];
    filteredData = [];
    ...
    showToast('כל הקבצים נמחקו בהצלחה');
  }
}